...
class RemoteResourceCollection implements LoggerAwareInterface
{
     ...
     */
    public function __construct(array $resources, ?ResourceFactory $resourceFactory = null, ?FileRepository $fileRepository = null, ?StorageRepository $storageRepository = null)
    {
    ...
    protected function getFileObjectFromStorage(ResourceStorage $storage, string $fileIdentifier): ?FileInterface
    {
        if (!$storage->isWithinProcessingFolder($fileIdentifier)) {
            try {
                $fileObject = $this->resourceFactory->getFileObjectFromCombinedIdentifier($storage->getUid() . ':' . $fileIdentifier);
